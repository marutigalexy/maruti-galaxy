...
describe("entry schemas", () => {
  ...
  it("records job payments and requires the selected party invoice", () => {
    ...
    expect(employeePayment).toMatchObject({
      employee_id: UUID,
      amount: "120.00",
    });
...
describe("entry and allocation security", () => {
  ...
  const employeeDialog = readFileSync(
    path.join(process.cwd(), "src/components/employees/employee-payment-dialog.tsx"),
    "utf8",
  );
  ...
  it("authorizes every entry and allocation method and uses the allocation RPC", () => {
    ...
    expect(actions).toMatch(/parseOrThrow\(createEmployeePaymentSchema/);
